...
const MovieInfoRight = ({ movie }) => {
  ...
  const { title, tagline, overview, original_language, runtime,release_date } = movie;
  return (
    <div className="bg-red-300 rounded-2xl  gap-1  p-3 md:p-6 md:mx-4 flex flex-col justify-start">
      <div className="text-3xl font-bold pb-2 border-b-2">{title}</div>
      {tagline && (
        <div className="tagLine  md:text-sm font-semibold  ps-1  ">
          {tagline}{" "}
        </div>
      )} 
      ...
      <div className=" py-1 rounded-lg text-md">
        Run-Time :  <span className="font-bold">{runtime} Minutes/{(runtime / 60).toFixed(1)} Hrs{" "}</span>
      </div>
      <div className=" py-1 rounded-lg  text-md">
        Original Language : <span className="font-bold">{original_language.toUpperCase()} </span>
      </div>
      <div className=" py-1 rounded-lg  text-md">
      ...
      <div className="flex flex-col gap-1 justify-start items-start border-t-2 pt-3">
        ...
        <div className="movieClips px-4 rounded-xl  flex py-2 gap-5 w-96 md:w-[38rem]   overflow-scroll items-center ">
